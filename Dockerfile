# Build: docker build -t teego/gitlab:7.9.4 .
# Data: docker run --name gitlab_data --volume /var/opt/gitlab --volume /var/log/gitlab --volume /etc/gitlab ubuntu:14.04 /bin/true
# Run: docker run --detach --name gitlab_app --publish 12080:80  --publish 12443:80 --publish 12022:22 --volumes-from gitlab_data teego/gitlab:7.9.4
...
RUN TMP_FILE=$(mktemp); \
    wget -q -O $TMP_FILE https://downloads-packages.s3.amazonaws.com/ubuntu-14.04/gitlab_7.9.4-omnibus-1_amd64.deb \
    && dpkg -i $TMP_FILE \
    && rm -f $TMP_FILE
